...
const VolunteerDeliveries = () => {
  ...
  const handleDeliver = async (id) => {
    try {
      await requestService.deliver(id);
      toast.success('Delivery completed! +10 reward points');
      load();
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to update');
    }
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-bold text-dark dark:text-white mb-1">My Deliveries</h1>
      <p className="text-sm text-slate-500 mb-8">Track pickups from acceptance to delivery.</p>

      {loading ? (
        <div className="space-y-3">{[1, 2, 3].map((i) => <div key={i} className="skeleton h-20" />)}</div>
      ) : requests.length === 0 ? (
        <div className="glass-card p-10 text-center text-slate-500">No deliveries in progress.</div>
      ) : (
          ...
          {requests.map((r) => (
            <div key={r._id} className="glass-card p-5 flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <p className="text-xs text-slate-500">Donor: {r.donor?.name} • {r.donation?.pickupAddress}</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                {r.status === 'collected' && (
                  <button onClick={() => handleDeliver(r._id)} className="btn-primary !py-1.5 !px-4 text-xs">
                    Mark Delivered
                  </button>
                )}
              </div>
